import {
  IconBrandAws,
  IconBrandGoogle,
  IconBrandGoogleAnalytics,
  IconBrandJavascript,
  IconBrandNextjs,
  IconBrandNodejs,
  IconBrandReact,
  IconBrandTailwind,
  IconBrandTypescript,
  IconBrandVercel,
  IconCloud,
  IconDatabase,
  IconLock,
  IconPlugConnected,
  IconShield,
  IconTerminal,
  IconTrain,
} from "@tabler/icons-react";

// ==================== FREELANCE PROJECTS ====================
export const FREELANCE_PROJECTS = [
  {
    title: "RankFlys - Education (Neet & Jee)",
    description:
      "Complete education platform for NEET & JEE aspirants with payment integration, course management, and AWS deployment.",
    tech: [
      { icon: IconBrandNodejs, name: "Node.js" },
      { icon: IconBrandNodejs, name: "Express" },
      { icon: IconDatabase, name: "PostgreSQL" },
      { icon: IconShield, name: "Razorpay" },
      { icon: IconDatabase, name: "Prisma" },
      { icon: IconBrandAws, name: "AWS" },
      { icon: IconBrandReact, name: "NextUI" },
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandTypescript, name: "TypeScript" },
      { icon: IconBrandTailwind, name: "Tailwind" },
      { icon: IconLock, name: "JWT" },
    ],
    link: "https://rankflys.vercel.app/",
    cover: "/project-images/rankflys.png",
    tags: ["Full Stack", "Education", "Payment Gateway"],
  },
  {
    title: "SteelOnMobile - Brochure",
    description:
      "Modern brochure website for steel industry with responsive design, content management, and SEO optimization.",
    tech: [
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandTypescript, name: "TypeScript" },
      { icon: IconBrandTailwind, name: "Tailwind" },
      { icon: IconLock, name: "JWT" },
      { icon: IconBrandTailwind, name: "shadcn/ui" },
      { icon: IconDatabase, name: "MongoDB" },
      { icon: IconDatabase, name: "Mongoose" },
    ],
    link: "https://www.steelonmobile.com/",
    cover: "/project-images/steelonmobile.png",
    tags: ["Frontend", "Brochure", "SEO"],
  },
];

// ==================== PERSONAL PROJECTS ====================
export const PERSONAL_PROJECTS = [
  {
    title: "Youth AF",
    subtitle: "Computer Science Learning Platform",
    description:
      "YouthAF is a learning-management and resource platform for computer-science education. It helps students discover public resources, view notes and PYQs, manage access to purchased material, complete their learning profile, and contact the team when needed. Success means the app feels fast, direct, trustworthy, and easy to scan across public and authenticated workflows.",
    tech: [
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandReact, name: "TanStack Start" },
      { icon: IconBrandReact, name: "React" },
      { icon: IconBrandTypescript, name: "TypeScript" },
      { icon: IconBrandNodejs, name: "Elysia" },
      { icon: IconBrandNodejs, name: "Bun" },
      { icon: IconPlugConnected, name: "Drizzle ORM" },
      { icon: IconDatabase, name: "PostgreSQL" },
      { icon: IconTrain, name: "Railway" },
      { icon: IconBrandGoogle, name: "Google Adsense" },
      { icon: IconBrandGoogleAnalytics, name: "PostHog" },
      { icon: IconBrandAws, name: "AWS S3" },
      { icon: IconShield, name: "Razorpay" },
      { icon: IconBrandTailwind, name: "Tailwind CSS" },
    ],
    link: "https://youth-af.com",
    cover: "project-images/youth-af.png",
    tags: ["Education", "Computer Science", "Full Stack", "Next.js"],
    highlighted: true,
    stats: [
      { label: "Daily Visitors", value: "1,000+" },
      { label: "Lifetime Visitors", value: "200K+" },
      { label: "Total Page Views", value: "1M+" },
    ],
  },
  {
    title: "CeroCode",
    subtitle: "AI-Powered CLI Tool for Developers",
    description:
      "Modern, production-ready command-line interface that brings conversational AI directly to your terminal with enterprise-grade architecture. Features real-time LLM streaming, autonomous agentic behavior with internet search and code execution, and seamless device code OAuth authentication. Built on a serverless streaming architecture using Server-Sent Events (SSE) and Inngest Realtime pub/sub for zero-polling, low-latency responses. The CLI communicates with a Next.js API backend deployed on Vercel, leveraging Vercel AI SDK for multi-provider LLM support and Better Auth for secure session management. Includes persistent conversation history, token refresh mechanisms, and cross-platform credential storage using OS-native keychains (macOS Keychain, Windows Credential Manager, Linux Secret Service).",
    tech: [
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandTypescript, name: "TypeScript" },
      { icon: IconBrandNodejs, name: "Node.js" },
      { icon: "/inngest.png", name: "Inngest", type: "image" },
      { icon: IconCloud, name: "Vercel AI SDK" },
      { icon: IconTerminal, name: "CLI (Commander)" },
      { icon: IconLock, name: "Better Auth" },
      { icon: IconDatabase, name: "PostgreSQL" },
      { icon: IconBrandVercel, name: "Vercel" },
    ],
    link: "https://cerocode.heyabhishek.in",
    cover: "/project-images/cero.png",
    tags: ["AI", "CLI Tool", "Developer Tools", "LLM"],
  },

  {
    title: "Lunox.ai",
    subtitle: "Web Search + LLM Summarization",
    description:
      "AI-powered search engine that combines web search results with LLM summarization using Google Gemini and Brave Search API.",
    tech: [
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandReact, name: "React" },
      { icon: IconBrandTypescript, name: "TypeScript" },
      { icon: IconBrandTailwind, name: "Tailwind" },
      { icon: "/inngest.png", name: "Inngest", type: "image" },
      { icon: IconCloud, name: "Gemini AI" },
      { icon: IconCloud, name: "Brave Search" },
      { icon: IconDatabase, name: "Drizzle ORM" },
      { icon: IconDatabase, name: "PostgreSQL" },
    ],
    link: "https://lunox.heyabhishek.in",
    cover: "/project-images/lunox.png",
    tags: ["AI", "Search Engine", "Full Stack"],
  },
  {
    title: "Pencil.io",
    subtitle: "Documents & Diagrams",
    description:
      "Collaborative document editor and diagram creator with real-time synchronization and Stripe payment integration.",
    tech: [
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandReact, name: "React" },
      { icon: IconBrandTypescript, name: "TypeScript" },
      { icon: IconBrandJavascript, name: "JavaScript" },
      { icon: IconBrandTailwind, name: "Tailwind" },
      { icon: IconLock, name: "JWT" },
      { icon: IconDatabase, name: "MongoDB" },
      { icon: IconShield, name: "Stripe" },
    ],
    link: "https://pencil-io.heyabhishek.in",
    cover: "/project-images/pencil-io.png",
    tags: ["Collaboration", "SaaS", "Payment"],
  },
  {
    title: "Event Ocean",
    subtitle: "Your Events, Our Platform",
    description:
      "Complete event management platform with ticket booking, payment processing, and event discovery features.",
    tech: [
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandReact, name: "React" },
      { icon: IconBrandTypescript, name: "TypeScript" },
      { icon: IconBrandJavascript, name: "JavaScript" },
      { icon: IconBrandTailwind, name: "Tailwind" },
      { icon: IconDatabase, name: "MongoDB" },
      { icon: IconShield, name: "Stripe" },
    ],
    link: "https://event-ocean.heyabhishek.in/",
    cover: "/project-images/event-ocean.png",
    tags: ["Event Management", "Tickets", "SaaS"],
  },
  {
    title: "PromptHub",
    subtitle: "Discover & Share AI Prompts",
    description:
      "Community platform for discovering, sharing, and rating AI prompts for various use cases and models.",
    tech: [
      { icon: IconBrandNextjs, name: "Next.js" },
      { icon: IconBrandReact, name: "React" },
      { icon: IconBrandJavascript, name: "JavaScript" },
      { icon: IconBrandTailwind, name: "Tailwind" },
      { icon: IconLock, name: "JWT" },
      { icon: IconDatabase, name: "MongoDB" },
    ],
    link: "https://prompt-hub.heyabhishek.in",
    cover: "/project-images/prompt-hub.png",
    tags: ["AI", "Community", "Prompts"],
  },
  //   {
  //     title: "Internhub",
  //     subtitle: "College Placement Cell",
  //     description:
  //       "Complete placement management system for colleges with student profiles, job postings, and application tracking.",
  //     tech: [
  //       { icon: IconBrandNextjs, name: "Next.js" },
  //       { icon: IconBrandReact, name: "React" },
  //       { icon: IconBrandJavascript, name: "JavaScript" },
  //       { icon: IconBrandTailwind, name: "Tailwind" },
  //       { icon: IconLock, name: "JWT" },
  //       { icon: IconDatabase, name: "MongoDB" },
  //       { icon: IconBrandNodejs, name: "Express" },
  //     ],
  //     link: "https://internhub.in",
  //     cover: "/project-images/internhub.png",
  //     tags: ["Placement", "College", "Management"],
  //   },
  //   {
  //     title: "Flixify",
  //     subtitle: "Watch Movies",
  //     description:
  //       "Movie streaming platform with extensive catalog, search functionality, and personalized recommendations.",
  //     tech: [
  //       { icon: IconBrandNextjs, name: "Next.js" },
  //       { icon: IconBrandReact, name: "React" },
  //       { icon: IconBrandJavascript, name: "JavaScript" },
  //       { icon: IconBrandTailwind, name: "Tailwind" },
  //       { icon: IconLock, name: "JWT" },
  //       { icon: IconDatabase, name: "MongoDB" },
  //     ],
  //     link: "https://flixify-ruby.vercel.app/",
  //     cover: "/project-images/flixify.png",
  //     tags: ["Streaming", "Entertainment", "Frontend"],
  //   },
  //   {
  //     title: "Livecode",
  //     subtitle: "Collaborative Coding",
  //     description:
  //       "Real-time collaborative code editor with syntax highlighting, live cursors, and instant synchronization.",
  //     tech: [
  //       { icon: IconBrandNextjs, name: "Next.js" },
  //       { icon: IconBrandReact, name: "React" },
  //       { icon: IconBrandJavascript, name: "JavaScript" },
  //       { icon: IconBrandTailwind, name: "Tailwind" },
  //       { icon: IconLock, name: "JWT" },
  //       { icon: IconDatabase, name: "MongoDB" },
  //       { icon: IconBrandSocketIo, name: "Socket.io" },
  //     ],
  //     link: "https://livecode-liart.vercel.app/",
  //     cover: "/project-images/livecode.png",
  //     tags: ["Real-time", "Collaboration", "IDE"],
  //   },
  //   {
  //     title: "Word Web",
  //     subtitle: "Stay Curious",
  //     description:
  //       "Educational platform for vocabulary building and word learning with interactive features and progress tracking.",
  //     tech: [
  //       { icon: IconBrandNextjs, name: "Next.js" },
  //       { icon: IconBrandReact, name: "React" },
  //       { icon: IconBrandJavascript, name: "JavaScript" },
  //       { icon: IconBrandTailwind, name: "Tailwind" },
  //       { icon: IconLock, name: "JWT" },
  //       { icon: IconDatabase, name: "MongoDB" },
  //       { icon: IconBrandNodejs, name: "Express" },
  //     ],
  //     link: "https://word-web-sigma.vercel.app/",
  //     cover: "/project-images/word-web.png",
  //     tags: ["Education", "Full Stack", "Learning"],
  //   },
  //   {
  //     title: "Medicare",
  //     subtitle: "Online Doctor Appointments",
  //     description:
  //       "Healthcare platform for booking doctor appointments, managing patient records, and telemedicine consultations.",
  //     tech: [
  //       { icon: IconBrandReact, name: "React" },
  //       { icon: IconBrandJavascript, name: "JavaScript" },
  //       { icon: IconBrandTailwind, name: "Tailwind" },
  //     ],
  //     link: "/",
  //     cover: "/project-images/medicare.png",
  //     tags: ["Healthcare", "Frontend", "Booking"],
  //   },
];
